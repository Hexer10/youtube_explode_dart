...
class PlaylistClient {
  ...
  Stream<Video> getVideos(dynamic id, {int maxRequests = 30}) async* {
    ...
    while (page != null && requestCount < maxRequests) {
      ...
      print(
          '📥 Request #${requestCount + 1}: +${videosAddedThisPage} videos (total: ${encounteredVideoIds.length})');
      ...
      try {
        ...
        if (nextPage.videos.isNotEmpty &&
          ...
          print(
              '⚠️ Detected duplicate page. Stopping to prevent infinite loop.');
          break;
        }
    ...
    if (requestCount >= maxRequests) {
      print(
          '⚠️ Reached maximum request limit ($maxRequests). There might be more videos.');
    }

    print(
        '🎉 Playlist fetch completed. Total videos: ${encounteredVideoIds.length}');
  }
